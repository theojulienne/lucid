...
typedef fastdelegate::FastDelegate<void (void *)> LDelegate;
...
public:
    ...
    static void Invoke(LDispatchList * list);
...
inline void LDispatchList::Invoke(LDispatchList * list)
{
    while(list)
    {
        list->m_func(list->m_user_data);
        list = list->m_next;
    }                       
}
...
{
    printf("%d\n", sizeof(LDispatchList));
    LDispatchList * list = NULL, * elem = NULL;
    list = LDispatchList::Add(list, LDelegate(foo), (void *)0xdeadbeef, foo_del);
    elem = list;
    list = LDispatchList::Add(list, LDelegate(foo), (void *)0xc0ffee, foo_del);
    list = LDispatchList::Remove(list, (LDispatchList *)elem);    
    LDispatchList::Invoke(list);
